...
The subtlest methodological question in DP is not how much noise to add but where the privacy boundary sits. *Boenisch et al.* (2023) interrogate the popular pipeline that combines **federated learning, central DP, and secure aggregation** and is widely advertised as privacy-preserving. They show the composition is brittle: a malicious server controlling model initialisation and aggregation can **reconstruct individual training points** even though it only observes a sum of clipped, noised client updates. Secure aggregation hides individual updates but not the *function* the server computes on them; by adversarially shaping the global model between rounds, the server engineers aggregations that collapse onto a single client. The paper reframes DP-FL as a problem of *protocol design*: the right $\varepsilon$ accounting is meaningless if the threat model under which it was proved does not match the deployment.
