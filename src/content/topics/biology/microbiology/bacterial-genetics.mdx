---
title: "Bacterial Genetics"
description: "Plasmids, transformation, conjugation, transduction, and lateral gene transfer."
parent: biology/microbiology/bacteriology
order: 1
color: '#16a34a'
difficulty: intermediate
prerequisites:
  - biology/microbiology/bacteriology
status: draft
reviewTier: field
sources:
  - type: textbook
    title: "Molecular Genetics of Bacteria"
    authors:
      - snyder-larry-2013
      - peters-joseph-2013
      - henkin-tina-2013
      - champness-2013
    year: 2013
    role: primary
  - type: textbook
    title: "Brock Biology of Microorganisms"
    authors:
      - madigan-2021
      - bender-2021
      - buckley-daniel-2021
      - sattley-2021
      - stahl-david-2021
    year: 2021
    role: primary
author: agent
lastEditedBy: agent
lastUpdated: '2026-05-12'
---

**Bacterial Genetics** sits within bacteriology and addresses plasmids, transformation, conjugation, transduction, and lateral gene transfer. The page below sketches the conceptual scope of the area, the methodological tools it relies on, and the recent literature anchoring its current frontier.

The area organises around a small number of recurring axes: *scope* (what biological scales the work spans), *method* (the dominant experimental or computational tools), *data regime* (what kinds of measurements are now routine vs. still frontier), and *open questions* (what the field cannot yet do reliably). The sources below cover different combinations of these axes.

## Foundational references

*Snyder, Molecular Genetics of Bacteria* is a standard reference for the foundations covered here, used across the field to anchor terminology, canonical models, and the relationships between sub-areas of bacterial genetics. Treat it as the entry point to which the more specialised work below adds frontier detail.

*Madigan, Brock Biology of Microorganisms* is a standard reference for the foundations covered here, used across the field to anchor terminology, canonical models, and the relationships between sub-areas of bacterial genetics. Treat it as the entry point to which the more specialised work below adds frontier detail.

## Open questions

Open questions in bacterial genetics cluster around scaling current methods to larger systems, integrating measurements across modalities, and producing predictive rather than descriptive models. The references above mark the work that the next iteration of this page should engage with in more specific detail.
