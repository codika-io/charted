---
title: "Community Ecology"
description: "Coexistence, competition, predation, and food-web structure."
parent: biology/ecology
order: 2
color: '#16a34a'
difficulty: intermediate
prerequisites:
  - biology/ecology
status: draft
reviewTier: foundation
sources:
  - type: textbook
    title: "Ecology: From Individuals to Ecosystems"
    authors:
      - begon-2021
      - townsend-colin-2021
      - harper-john-2021
    year: 2021
    role: primary
  - type: textbook
    title: "Resource Competition and Community Structure"
    authors:
      - tilman-1982
    year: 1982
    role: primary
author: agent
lastEditedBy: agent
lastUpdated: '2026-05-12'
---

**Community Ecology** sits within ecology and addresses coexistence, competition, predation, and food-web structure. The page below sketches the conceptual scope of the area, the methodological tools it relies on, and the recent literature anchoring its current frontier.

The area organises around a small number of recurring axes: *scope* (what biological scales the work spans), *method* (the dominant experimental or computational tools), *data regime* (what kinds of measurements are now routine vs. still frontier), and *open questions* (what the field cannot yet do reliably). The sources below cover different combinations of these axes.

## Foundational references

*Begon, Ecology: From Individuals to Ecosystems* is a standard reference for the foundations covered here, used across the field to anchor terminology, canonical models, and the relationships between sub-areas of community ecology. Treat it as the entry point to which the more specialised work below adds frontier detail.

*Tilman, Resource Competition and Community Structure* is a standard reference for the foundations covered here, used across the field to anchor terminology, canonical models, and the relationships between sub-areas of community ecology. Treat it as the entry point to which the more specialised work below adds frontier detail.

## Open questions

Open questions in community ecology cluster around scaling current methods to larger systems, integrating measurements across modalities, and producing predictive rather than descriptive models. The references above mark the work that the next iteration of this page should engage with in more specific detail.
